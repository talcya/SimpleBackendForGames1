...
describe('rule engine unit (fast)', () => {
  ...
  afterAll(async () => {
    try {
      ...
      // eslint-disable-next-line no-console
      console.warn('mongoose disconnect failed in unit test cleanup:', (err as any) && (err as any).message ? (err as any).message : err);
    }
  });

  it('creates a Violation for a single event when threshold is 1', async () => {
    const user = await UserModel.create({ displayName: `unit-${Date.now()}-${Math.random().toString(36).slice(2,8)}` });
